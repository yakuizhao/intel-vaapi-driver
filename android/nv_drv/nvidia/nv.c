...
void *nv_alloc_file_private(void)
{
    ...
    nvfp->deviceInstance = NV_MAX_DEVICES;
...
static irqreturn_t nvidia_soc_isr(int irq, void *arg)
{
    ...
    {
        if (nv->soc_irq_info[irq_count].bh_pending == true)
        {
    ...
    if (ret == IRQ_WAKE_THREAD)
    {
        for (irq_count = 0; irq_count < nv->num_soc_irqs; irq_count++)
        {
            ...
            {
                nv->soc_irq_info[irq_count].bh_pending = true;
            }
        }
    }
    else
...
)
{
    ...
    //
    for (i = 0; i < nvfp->numExportedRmObjects; i++)
    {
        if (nvfp->hExportedRmObject[i] != 0)
        {
            rm_free_exported_object(sp, nvfp->hExportedRmObject[i]);
        }
    }
    nvfp->numExportedRmObjects = 0;
...
)
{
    ...
    if ((nvfp->hExportedRmObject[i] != 0) ||
        (nvfp->deviceInstance != NV_MAX_DEVICES && nvfp->deviceInstance != deviceInstance))
    {
    ...
    nvfp->hExportedRmObject[i] = hExportedRmObject;
    nvfp->deviceInstance = deviceInstance;
    nvfp->numExportedRmObjects++;
done:
...
)
{
    ...
    if ((nvfp->hExportedRmObject[i] == 0) && (i < nvfp->numExportedRmObjects))
    {
        status = NV_ERR_INVALID_ARGUMENT;
        goto done;
    }

    *pExportedObject = nvfp->hExportedRmObject[i];
...
#define SNAPSHOT_TIMER_FREQ (jiffies + HZ / 100) /* 10ms */
...
void NV_API_CALL nv_control_soc_irqs(nv_state_t *nv, NvBool bEnable)
{
    ...
    if (bEnable)
    {
        for (count = 0; count < nv->num_soc_irqs; count++)
        {
            nv->soc_irq_info[count].bh_pending = false;
            nv->current_soc_irq = -1;

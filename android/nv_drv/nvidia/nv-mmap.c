...
)
{
    ...
    NvU64 pfn_start =
        (nvfp->mmap_context.mmap_start >> PAGE_SHIFT) + vma->vm_pgoff;
    ...
    if (!nvl->safe_to_mmap)
    {
        ...
        nvl->gpu_wakeup_callback_needed = FALSE;
    ...
    for (page = 0; page < num_pages; page++)
    {
        ...
        ret = nv_insert_pfn(vma, virt_addr, pfn,
                            nvfp->mmap_context.remap_prot_extra);
        ...
        nvl->all_mappings_revoked = FALSE;
    }
...
)
{
    ...
    //
    if (nvfp->numExportedRmObjects != 0)
    {
        return -EINVAL;
    }
...
)
{
    ...
    list_for_each_entry (nvfp, &nvl->open_files, entry)
    {
        unmap_mapping_range(&nvfp->mapping, 0, ~0, 1);
    }
